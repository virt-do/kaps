...
impl ContainerState {
    ...
    fn _new(id: &str, bundle_path: &str, container_dir: &str) -> Result<Self> {
        ...
        container_state._save(container_dir)?;
    ...
    pub fn save(&self) -> Result<()> {
        self._save(KAPS_ROOT_PATH)
    }

    fn _save(&self, container_dir: &str) -> Result<()> {
        ...
        let serialized = serde_json::to_string_pretty(&self).map_err(Error::SerializeState)?;

        file.write_all(serialized.as_bytes())
            .map_err(Error::WriteStateFile)?;
...
mod tests {
    ...
    fn should_update_runtime_status() -> Result<()> {
        ...
        let file_state: ContainerState =
            serde_json::from_str(&file_state).map_err(Error::SerializeState)?;
